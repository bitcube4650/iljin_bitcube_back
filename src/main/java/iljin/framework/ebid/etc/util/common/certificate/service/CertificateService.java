...
public class CertificateService {
	...
	public ResultBody checkCert(String signedData) {
		...
		System.out.println("checkCert 들어옴");
		try {
			JeTS.installProvider(Constances.CERTIFICATE_SETTING_PATH);
			System.out.println("checkCert 들어옴1");
			//서명된 데이터를 byte로 전환
			String message = new String(signedData.getBytes("ISO-8859-1"));
			...
			System.out.println("인증서 길이" + certs.length);
			System.out.println("원래 서명된거" + sd.getContent());
			...
			for (int i = 0; i < certs.length; i++)
			{
				...
				if(r){
					...
					System.out.println(crl.getRevokedReason());
					return resultBody;
				}	
				else{
