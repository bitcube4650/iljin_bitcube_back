...
//				data = excelRepository.findComplateBidList(param, offset, limit);
	...
	public SXSSFWorkbook getWorkBookPaging(String interrelatedCustCode, SXSSFWorkbook workbook, List<String> headerNames, Map<String, Object> param) throws IllegalAccessException, IOException {
		...
		if("02".equals(interrelatedCustCode)) {			 //입찰이력 롯데머트리얼즈
			createHeadersBidHisMat(workbook, row, cell, headerNames);
		} else {
			createHeadersBidHis(workbook, row, cell, headerNames);
		}
...
//		int cnt = 0;
		...
		while(offset < complateBidListCnt) {
			...
			String tmpBiNo = "";
			List<Object> dataList = excelRepository.findComplateBidList(param, offset, limit);
			...
			for(int i = 0; i < dataList.size(); i++) {
				...
				if(!tmpBiNo.equals(data.get("biNo"))) {
					...
					if("02".equals(interrelatedCustCode)) {
						...
						dto.put("matProc", data.get("matProc"));
						dto.put("matCls", data.get("matCls"));
						dto.put("matFactory", data.get("matFactory"));
						dto.put("matFactoryLine", data.get("matFactoryLine"));
						dto.put("matFactoryCnt", data.get("matFactoryCnt"));
					}
					dto.put("biName", data.get("biName"));
					dto.put("bdAmt", data.get("bdAmt"));
					dto.put("succAmt", data.get("succAmt"));
					dto.put("custName", data.get("custName"));
					dto.put("estStartDate", data.get("estStartDate"));
					dto.put("estCloseDate", data.get("estCloseDate"));
					dto.put("userName", data.get("userName"));
			...
			createbody(workbook, excelData, sheet, row, cell, start);
	...
	public void createbody(Class<?> clazz, SXSSFWorkbook workbook, List<?> data, Sheet sheet, Row row, Cell cell, int rowNo) throws IllegalAccessException {
		...
		// 통화 서식 생성
		short currencyFormat = workbook.createDataFormat().getFormat("#,###");
